...
/** Short preview for article cards (homepage / listing). */
export function truncateArticleExcerpt(text, maxLength = 150) {
...
/** Curated Mersin guide collection shown on the homepage. */
export const MERSIN_GUIDE_SLUGS = [
...
export function isMersinGuideArticle(slug) {
  return MERSIN_GUIDE_SLUGS.includes(slug);
}

export function getMersinGuideArticles() {
  const bySlug = new Map(getAllArticles().map((article) => [article.slug, article]));
  return MERSIN_GUIDE_SLUGS.map((slug) => bySlug.get(slug)).filter(Boolean);
}
